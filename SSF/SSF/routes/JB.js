				...
				function (err, result) {
					...
					if (result != null) {
						if (result['type'] == type && type == 'public') {
							...
							result['type'] == type &&
							type == 'private' &&
							result['password_public'] == req.body.board_password
						) {
							jumpboard(req, res, result['type']);
						} else warming(res, 1);
					} else warming(res, 3);
...
router.post('/to_post_page', async function (req, res) {
	//console.log(req.body);
	res.render('Page9', {
		ID: req.body.ID,
		board_ID: req.body.board_ID,
		type: req.body.type,
		personal_password: await encrypt(req.body.ID, req.body.personal_password),
