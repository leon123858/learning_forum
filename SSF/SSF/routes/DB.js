				...
				function (err, result) {
					...
					if (result != null) {
						if (result['password'] == req.body.personal_password) {
							if (result.activate) callback(req, res);
							else warming(res, 0);
						} else warming(res, 1);
					} else warming(res, 3);
				}
		...
		function (err, db) {
			...
			table.findOneAndUpdate(
				...
				function (err, result) {
					...
					db.close();
					if (result.ok == 1) add_lockForboardID(req, res, result.value.count);
					else warming(res, 2);
				}
			);
		}
...
function jumpPublic(req, board_ID, ID, res) {
	MongoClient.connect(
		...
		function (err, db) {
			...
			table
				.find(findThing, { projection: { _id: 0 } })
				.sort({ _id: 1 })
				.toArray(async function (err, result) {
					...
					var pkg = {
						...
						password: await encrypt(ID, req.body.personal_password),
					};
					if (req.body.place) pkg['place'] = req.body.place;
					else pkg['place'] = 'NA';
		...
		function (err, db) {
			...
			table.findOneAndUpdate(
				...
				function (err, result) {
					...
					db.close();
					//console.log(result);
					if (result.ok == 1) discuss_update(req, res, result.value.count);
					else warming(res, 3);
				}
			);
		}
		...
		function (err, db) {
			...
			table.findOne(
				...
				function (err, result) {
					...
					if (result != null) discuss_notice_add(req, result);
					else warming(res, 3);
				}
			);
		}
				...
				function (err, result) {
					...
					if (result != null) {
						if (result['password'] == req.body.password) {
							if (result.activate) to_rewrite_check_numID(req, res);
							else warming(res, 0);
						} else warming(res, 1);
					} else warming(res, 1);
				}
	...
	MongoClient.connect(
		...
		function (err, db) {
			...
			table.findOne(
				findThing,
				{
					projection: { _id: 0, num: 0, ID: 0, title: 0, include: 0, hide: 0 },
				},
				function (err, result) {
					if (err) {
