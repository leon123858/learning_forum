...
const { GetUrl, GetSAN } = require('./database_url');
...
function randomString(len) {
	len = len || 32;
	var $chars =
		'ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678'; /****預設去掉了容易混淆的字元oOLl,9gq,Vv,Uu,I1****/
...
function sendEmail(req, res, mailTransport, title, include) {
	...
	var mailOtions = {
		...
		//text: 'XXXX', // 單純文字內容
		html:
			'<h1>親愛的使用者您好</h1><h2>我們是高中教師社會科學增能平台, 您註冊了本平台會員, 但帳號屬於未啟用狀態,點擊下方連接, 啟用留言與發文功能。</h2>' +
			'<a href="' +
			URL +
			'">請點我(Click ME) [啟用後帳號可以獲得發文與留言權限]</a>', // 可寫入 HTML 格式
	};
				...
				function (err, result) {
					...
					mailTransport.sendMail(mailOtions, function (error, info) {
						...
						res.render('Page1', {
							...
							include: include,
							result:
								'帳號註冊成功, 需進行e-mail認證才可以進行留言與發文, 且若長時間未進行郵箱認證將刪除此帳號',
						});
					});
				}
			);
		...
		MongoClient.connect(
			...
			function (err, found_connect) {
				...
				found_database.collection('personal_information').findOne(
					...
					async function (err, found_personal_information) {
						if (err) {
							warming(res, 2);
							throw err;
						}

						if (found_personal_information == null) {
							//No user in database
							...
							if (found_personal_information['password'] == req.body.password) {
								//Right password
								found_personal_information['password'] = await encrypt(
									req.body.ID,
									req.body.password
								);
...
function checkInput(str) {
	const illegalChars = new Array('<', '>', '.', '!', '_', '*'); //列出所有被禁止的方法字元
	for (let i = 0; i < str.length; i++) {
		if (illegalChars.includes(str.charAt(i))) return false;
	}
	return true;
}
				...
				found_database
					...
					.findOne(
						...
						function (err, found_personal_information) {
							...
							if (!(checkInput(req.body.password) && checkInput(req.body.ID))) {
								var pkg = {
									ID: req.body.ID,
									...
									include: '<h1>重新進入網站可以再一次看到介紹</h1>',
									result: '不可使用特殊字元於帳號密碼',
									title: '不可使用特殊字元於帳號密碼',
								};
								router_result.render('Page1', pkg);
								found_connect.close();
							} else if (found_personal_information == null) {
								//No user in database
								//console.log("No user in database");
								...
								else pkg['title'] = '電子郵箱已存在 請更改郵箱再試一次';
								router_result.render('Page1', pkg);
								found_connect.close();
							}
						}
	...
	return new Promise((resolve, reject) => {
		...
		table.findOne(
			...
			function (err, result) {
				...
				if (result == null) resolve({ token: 'NA', mail: pkg.mail });
				else resolve({ token: result.token, mail: pkg.mail });
			}
		);
	});
}
					...
					found_database.collection('personal_information').findOne(
						...
						function (err, found_personal_information) {
							...
							if (found_personal_information == null) {
								...
								if (
									found_personal_information['password'] == req.body.password
								) {
									//Right password
									found_database.collection('personal_information').updateOne(
										{ ID: req.body.ID },
										{
											$set: goal,
										},
