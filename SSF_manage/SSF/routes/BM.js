		...
		function (err, db) {
			...
			table.findOneAndUpdate(
				...
				function (err, result) {
					...
					db.close();
					if (result.ok == 1) build_setLockboard(req, res, result.value.count);
					else warming(res, 2);
				}
			);
		}
...
function build_setLockboard(req, res, num) {
	MongoClient.connect(
		...
		function (err, db) {
			...
			var insertThing = {
				board_ID: req.body.type + '_' + num.toString(),
				count: 1,
			};
			table.insertOne(insertThing, function (err, result) {
				...
				db.close();
				//console.log(result);
				build_insertManagerInformation(
					req,
					res,
					req.body.type + '_' + num.toString()
				);
			});
		}
	);
	...
	MongoClient.connect(
		...
		function (err, db) {
			...
			var insertThing = {
				...
				introduce: req.body.introduce.replace(/<script.*?>.*?<\/script>/gi, ''),
				name: req.body.name,
	...
	MongoClient.connect(
		...
		function (err, db) {
			...
			table.updateOne(filter, { $set: goal }, function (err, result) {
				...
				//console.log(result);
				db.close();
				if (req.body.type == 'public') update_ClassBoard(req, res);
		...
		function (err, db) {
			...
			table.findOne(
				...
				function (err, result) {
					...
					db.close();
					if (result != null) res.json({ result: 'success', data: result });
					else warming(res, 3);
				}
			);
		}
